...
* Production deployments should be fronted by HTTPS and configured with secure headers (e.g., `X-Content-Type-Options: nosniff`, `X-Frame-Options: SAMEORIGIN`, `Referrer-Policy: same-origin`, `Permissions-Policy`, and HTTPS-only `Strict-Transport-Security`).
...
* If the app sits behind a reverse proxy that terminates TLS, set `SD_TRUST_PROXY_HEADERS=1` only when that proxy is trusted and controlled by you.
